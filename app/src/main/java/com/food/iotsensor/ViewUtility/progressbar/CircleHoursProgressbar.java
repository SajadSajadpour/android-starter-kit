 ...
 */
package com.food.iotsensor.ViewUtility.progressbar;
...
public class CircleHoursProgressbar extends TextView {
...
//    private
    ...
    public CircleHoursProgressbar(Context context) {
        this(context, null);
    }

    public CircleHoursProgressbar(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CircleHoursProgressbar(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initialize(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public CircleHoursProgressbar(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initialize(Context context, AttributeSet attributeSet) {
        ...
        typedArray.recycle();
//        float textSize  = attributeSet.getAttributeFloatValue(R.attr.text_size, 0f);

        Log.d("drawText", "--------->>"+getTextSize());
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        StaticLayout layout = new StaticLayout(text, textPaint, 1000, Layout.Alignment.ALIGN_NORMAL, 1.0F, 0.0F, true);
        // 这里的参数2，表示字符串的长度，当满2时，就会换行，也可以使用“\r\n”来实现换行
        canvas.save();
        canvas.translate(bounds.centerX(), textY/2);
        layout.draw(canvas);
        canvas.restore();
        ...
        canvas.drawArc(mArcRect, 0, 360 * progress / 24, false, mPaint);
    }
    ...
    private Runnable progressChangeTask = new Runnable() {
        ...
        public void run() {
            ...
            if (progress >= 0 && progress <= 24) {
                ...
                invalidate();
                postDelayed(progressChangeTask, timeMillis / 10);
            } else
