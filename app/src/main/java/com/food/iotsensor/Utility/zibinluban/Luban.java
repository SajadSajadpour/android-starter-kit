package com.food.iotsensor.Utility.zibinluban;
                    ...
                    public void run() {
                        try {
                            ...
                            if (Checker.isNeedCompress(mLeastCompressSize, path)) {
                                ...
                                if (tagBitmap != null) {
                                    tagBitmap.compress(Bitmap.CompressFormat.PNG, 100, baos);
                                    tagBitmap.recycle();
                                    stream = baos;
                                    baos.close();
                                    mHandler.sendMessage(mHandler.obtainMessage(MSG_COMPRESS_SUCCESS, result));
                                } else {
                                    mHandler.sendMessage(mHandler.obtainMessage(MSG_COMPRESS_ERROR, null));
                                    Activity activity = (Activity) context;
                                    activity.runOnUiThread(new Runnable() {
                                        @Override
                                        public void run() {
                                            RemindDialogUtil.showEasy((Activity) context, context.getString(R.string.get_data_fail));
                                        }
                                    });
                                }
                            }
